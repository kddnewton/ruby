...
// This function is defined in iseq.c because it touches a lot of internals over
// there.
...
{
    if (value != Qundef && ((flags & IMEMO_CONST_CACHE_SHAREABLE) || rb_ractor_main_p())) {
...
{
    VALUE compiled = ic->entry ? ic->entry->compiled : Qfalse;
    ...
    RB_OBJ_WRITE(ice, &ice->compiled, compiled);
    RB_OBJ_WRITE(ice, &ice->value, ruby_vm_const_missing_count > 0 ? Qundef : val);
